...
#define OPT2(STR0, STR1) else if( !strcmp( name, STR0 ) || !strcmp( name, STR1 ) )
    ...
    OPT("sar")
    {
        b_error = ( 2 != sscanf_s( value, "%d:%d", &p->vui.i_sar_width, &p->vui.i_sar_height ) &&
                    2 != sscanf_s( value, "%d/%d", &p->vui.i_sar_width, &p->vui.i_sar_height ) );
    }
    OPT("overscan")
    ...
    OPT("fps")
    {
        if( sscanf_s( value, "%d/%d", &p->i_fps_num, &p->i_fps_den ) == 2 )
            ;
        else
        {
    ...
    OPT2("filter", "deblock")
    {
        if( 2 == sscanf_s( value, "%d:%d", &p->i_deblocking_filter_alphac0, &p->i_deblocking_filter_beta ) ||
            2 == sscanf_s( value, "%d,%d", &p->i_deblocking_filter_alphac0, &p->i_deblocking_filter_beta ) )
        {
            p->b_deblocking_filter = 1;
        }
        else if( sscanf_s( value, "%d", &p->i_deblocking_filter_alphac0 ) )
        {
            p->b_deblocking_filter = 1;
    ...
    OPT("psy-rd")
    {
        if( 2 == sscanf_s( value, "%f:%f", &p->analyse.f_psy_rd, &p->analyse.f_psy_trellis ) ||
            2 == sscanf_s( value, "%f,%f", &p->analyse.f_psy_rd, &p->analyse.f_psy_trellis ) )
        { }
        else if( sscanf_s( value, "%f", &p->analyse.f_psy_rd ) )
        {
