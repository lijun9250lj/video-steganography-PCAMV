                         ...
                         int stride, int width, int height, int16_t *buf )
{
    ...
    for( y=0; y<height; y++ )
    {
        for( x=-2; x<width+3; x++ )
        {// Y
            int v = TAPFILTER(src,stride);//垂直滤波半像素点
...
// hpel_ref0[]记录了亚像素点依赖于哪些点。数组元素共有四个取值：0，1，2，3。
// 这四个值分别代表整数像素，水平半像素，垂直半像素，对角线半像素。
...
//获取运动矢量中亚像素的部分的数据
//可以是半像素数据或者1/4像素数据   返回的就是可以直接和编码数据进行比较的数据lijun，可能是整、半或1/4像素，有mv的值决定，
                         ...
                         int i_width, int i_height )
{
    ...
    uint8_t *src1 = src[hpel_ref0[qpel_idx]] + offset + ((mvy&3) == 3) * i_src_stride;//src[0]到src[3]分别为整、半、半、对角半，

    if( qpel_idx & 5 ) /* qpel interpolation needed *///如果需要1/4像素估计，这个在前面是没有预先算好的，则需要临时插值，lijun,而半像素是已经算好的
    {
		// src[4]中[0]存储是整像素数据，[1]存储是水平半像素数据，[2]存储是垂直半像素数据，[3]存储是对角线半像素数据
        uint8_t *src2 = src[hpel_ref1[qpel_idx]] + offset + ((mvx&3) == 3);
		// 1/4 像素线性内插
        pixel_avg( dst, *i_dst_stride, src1, i_src_stride,
                   src2, i_src_stride, i_width, i_height );
...
/*
* 根据原图像生成一个宽高都为原图一半的子图像，保存在frame->lowres中
*/
