...
// 根据运动矢量获取参考块，计算和原始宏块之间的代价加上运动矢量的代价
// 如果代价更小，保存相应的运动矢量和代价到bpred_
...
{
	// 获取宏块的像素宽和高{16, 16 }, { 16, 8 }, { 8, 16 }, { 8,  8 }, { 8,  4 }, { 4,  8 }, { 4,  4 }
    ...
    int bmx, bmy, bcost;
	// 用于记录预测运动矢量中的最优
    int bpred_mx = 0, bpred_my = 0, bpred_cost = COST_MAX; //这几个保存的是前面x264估计的几个候选mv-mvc的信息，与搜索无关，最后是这里面的最优和后面的搜索最优进行比较lijun
	// 应该保存额是整像素的运动矢量
    int omx, omy, pmx, pmy;
    uint8_t *p_fref = m->p_fref[0];//应该是指向参考帧同位置宏块的像素,整像素
	// 像素块
...
#define CHECK_MVRANGE(mx,my) ( mx >= mv_x_min && mx <= mv_x_max && my >= mv_y_min && my <= mv_y_max )
	//运动矢量的代价
    const int16_t *p_cost_mvx = m->p_cost_mv - m->mvp[0];//这是指针位移，查表操作，m->p_cost_mv指向原点，左边为负数mv的失真，右边为正数mv的失真，
    ...
    /* try extra predictors if provided(如果有的预测运动矢量) */
    if( h->mb.i_subpel_refine >= 3 )//如果精度为1/4  动态预测和分区方式，可选1-7，默认5，与压缩质量与关系密切，1是7速度的4倍以上，
    {
		// 将运动矢量的的水平和垂直分量pack成一个32比特的数，为了后面操作方便
        uint32_t bmv = pack16to32_mask(bmx,bmy);
        COST_MV_HPEL( bmx, bmy ); // 计算运动矢量对应的代价，这对应着预测mv的失真， 由两部分组成，1是ssd失真，2是mv所需的bits（这个是由一个原先计算好的数组直接查表）lijun
        for( i = 0; i < i_mvc; i++ )//这里应该是有多个预先获得的候选mv，分别进行代价计算，找一个小的，（不属于h264标准部分，是x264自己优化的） lijun
        {   // 如果候选的预测运动矢量不为0且与搜索起点对应的运动矢量不相等
            ...
            { //则使用这个运动矢量进行运动估计
                ...
                COST_MV_HPEL( mx, my );//计算运动矢量对应的代价
            }
        }
		//根据当前最优的运动矢量计算相应的代价保存到bcost,bmx,bmy 整数
        bmx = ( bpred_mx + 2 ) >> 2;
        bmy = ( bpred_my + 2 ) >> 2;//变为整像素  lijun
        COST_MV( bmx, bmy );// 整像素再做一遍，注意这里的mv单位是整像素，lijun
    }
    else
    {
        /* 根据预测的起点计算代价（对1/4像素进行四舍五入后） */
        COST_MV( pmx, pmy );
        ...
        for( i = 0; i < i_mvc; i++ )
        {
            ...
            int mx = (mvc[i][0] + 2) >> 2;
            int my = (mvc[i][1] + 2) >> 2;
            if( (mx | my) && ((mx-bmx) | (my-bmy)) )//这个地方是不是有点问题，应该也对(bmx+2)>>2 ???
            {
	...
	// 计算运动矢量（0,0）的代价
    COST_MV( 0, 0 ); 
	// 经过上述步骤，已经在多个预测的运动矢量中找到了最优的起点(bmx,bmy,bcost),整像素的
	...
	{//不同的运动估计算法作不同的处理
		case X264_ME_DIA:// 钻石搜索
		{
			...
			i = 0;
			do
			{   // 以（bmx，bmy）为起点搜索周围的四个点，最优保存到(bmx,bmy,bcost)
				DIA1_ITER(bmx, bmy);
				if ((bmx == omx) & (bmy == omy))// 最优的点和起点一样，退出
					break;
				if (!CHECK_MVRANGE(bmx, bmy)) // 越界，退出
					break;
			} while (++i < i_me_range);
			break;
		}
...
me_hex2:
		...
		for (i = 0; i < i_me_range / 2; i++)
		{
			...
			/* hexagon */
			// 搜索周围的6个点 costs中存放6个点的失真，找到一个最小的lijun
			/*
			* 
			*    2   3
			*
			...
			*    6   5
			*/
			COST_MV_X3_DIR(-2, 0, -1, 2, 1, 2, costs);//左边和下方的三个点
			COST_MV_X3_DIR(2, 0, 1, -2, -1, -2, costs + 3);//右边和上方的三个点
			...
			if (dir != -2)//这是一个标记，说明上面6个点中有一个新的点的cost比原来更小  ，以这个对应的dir值的点作为下一次的中心
			{
				//{-1,-2}, {-2,0}, {-1,2}, {1,2}, {2,0}, {1,-2}, {-1,-2}, {-2,0};六边形模板
				bmx += hex2[dir + 1][0];//以上面6个点中cost最小的点为中心，以搜索范围me_range/2为半径继续六边形搜索，直到找到：中心点的cost最小则完成第一步搜索lijun
				bmy += hex2[dir + 1][1];
				/* half hexagon, not overlapping the previous iteration */
				for (i = 1; i < i_me_range / 2 && CHECK_MVRANGE(bmx, bmy); i++)//
				{
					const int odir = mod6m1[dir + 1]; //{5,0,1,2,3,4,5,0}
					COST_MV_X3_DIR(hex2[odir + 0][0], hex2[odir + 0][1],  //比较3个点，因为这个新的六边形的另外3个点在之前已经比较过
					...
					if (dir == -2)//没有找到心得更小的，因此这个点就是最小的中心点，结束
						break;
					bmx += hex2[dir + 1][0];
					bmy += hex2[dir + 1][1];
				}
		...
		case X264_ME_UMH: //非对称十字型多层次六边形格点搜索算法
		{
...
#define SAD_THRESH(v) ( bcost < ( v >> x264_pixel_size_shift[i_pixel] ) )
		...
		case X264_ME_ESA: //连续消除法（ESA、TESA)，exhaustive search穷尽搜索lijun
		case X264_ME_TESA:
		{
    ...
    /* -> qpel mv */
    if( bpred_cost < bcost )//bpred保存的是前面x264估计的几个运动矢量的失真，而且bpred_cost是亚像素的值lijun
    {
        m->mv[0] = bpred_mx;//最终的最优
        m->mv[1] = bpred_my;
        m->cost = bpred_cost;
    }
    else
    {
        m->mv[0] = bmx << 2;//转换为1/4像素的mv
        m->mv[1] = bmy << 2;
        m->cost = bcost;
    }
	// 上述过程后，已经获得了整像素级别的最优运动矢量和代价，但是单位是1/4像素界别（乘了4）lijun
	...
	/* subpel refine */ //子像素精度（1/2，1/4）搜索
    if( h->mb.i_subpel_refine >= 2 )
    {
        int hpel = subpel_iterations[h->mb.i_subpel_refine][2];
        int qpel = subpel_iterations[h->mb.i_subpel_refine][3];
        refine_subpel( h, m, hpel, qpel, p_halfpel_thresh, 0 );//用于进行亚像素的运动搜索。add lijun
    }
...
static void refine_subpel( x264_t *h, x264_me_t *m, int hpel_iters, int qpel_iters, int *p_halfpel_thresh, int b_refine_qpel )
{
    ...
    for( i = hpel_iters; i > 0; i-- )
    {
		...
		*/
		...
		//得到 omx,omy周围的半像素4个点的地址.omx和omy以1/4像素为基本单位，+2或者-2取的就是半像素点
        ...
        if( (bmx == omx) & (bmy == omy) )// 如果最优的点和起点一样
            break;
    }
	...
	*/
    ...
    for( i = qpel_iters; i > 0; i-- )
    {
        ...
        omx = bmx;
        omy = bmy;
        COST_MV_SATD( omx, omy - 1, 0 );//依然是Diamond搜索
