...
void x264_mb_encode_i4x4( x264_t *h, int idx, int i_qp )
{
    ...
    if( nz ) //如果非0系数个数不为0，则需要进行重构
    {
		...
		//逆DCT变换并加上预测值,然后将结果放回到p_dec中，供以后参考使用
...
static void x264_mb_encode_i16x16( x264_t *h, int i_qp )
{
    ...
    {
        /* 复制DC系数 */
        dct_dc4x4[0][block_idx_xy_1d[i]] = dct4x4[i][0][0];	//每个4x4DCT块的[0]元素
        dct4x4[i][0][0] = 0;
    ...
    if( nz )
    {
        ...
        if( h->mb.i_cbp_luma )
            for( i = 0; i < 16; i++ )// 把DC系数重新赋值到每个DCT数组的[0]元素上
                dct4x4[i][0][0] = dct_dc4x4[0][block_idx_xy_1d[i]];
    }

    /* put pixels to fdec */
    if( h->mb.i_cbp_luma )//DCT反变换后，叠加到预测数据上（通用）
        h->dctf.add16x16_idct( p_dst, dct4x4 ); 
    else if( nz )//DCT反变换后，叠加到预测数据上（只有DC系数的时候
...
void x264_macroblock_encode( x264_t *h )
{
    ...
    int b_force_no_skip = 1; //原来是0， *****   重要lijun     这样，在编码阶段就不会将分析阶段的P_skip模式进行修改。
    int i,idx,nz;
    h->mb.i_cbp_luma = 0;
    h->mb.cache.non_zero_count[x264_scan8[24]] = 0; // DC 系数非零个数，
    ...
    else if( h->mb.i_type == I_4x4 )
    {
        ...
        for( i = h->mb.i_skip_intra ? 15 : 0 ; i < 16; i++ )
        {
			...
			// 使用相应的预测方法进行预测，结果保留在重建块中
            if( h->mb.b_lossless )
                x264_predict_lossless_4x4( h, p_dst, i, i_mode );
            else
                h->predict_4x4[i_mode]( p_dst );
            x264_mb_encode_i4x4( h, i, i_qp );//编码当前块，
        }
    }
    else    /* Inter MB */
    {
        ...
        else//p或b帧的情况，这个时候对应的重构块已经完全建好了放在了h->mb.pic.p_fdec中，
        {
            DECLARE_ALIGNED_16( int16_t dct4x4[16][4][4] );//存储当前宏块16个4*4块的所有dct系数
            h->dctf.sub16x16_dct( dct4x4, h->mb.pic.p_fenc[0], h->mb.pic.p_fdec[0] );//函数前带sub的是计算残差再dct变换，这里用重构p_fdec数据求差运算
            h->nr_count[0] += h->mb.b_noise_reduction * 16;

            for( i8x8 = 0; i8x8 < 4; i8x8++ )
            {
                int i_decimate_8x8 = 0;//破坏？
                int cbp = 0;
                ...
                for( i4x4 = 0; i4x4 < 4; i4x4++ )
                {
                    ...
                    nz = x264_quant_4x4( h, dct4x4[idx], i_qp, DCT_LUMA_4x4, 0, idx );//量化，得到非零系数个数
                    h->mb.cache.non_zero_count[x264_scan8[idx]] = nz;//保存非零系数的个数到缓存区

                    if( nz )//用于后面建立建立重建帧，这里只反量化
                    {
                        h->zigzagf.scan_4x4( h->dct.luma4x4[idx], dct4x4[idx] );
                        h->quantf.dequant_4x4( dct4x4[idx], h->dequant4_mf[CQM_4PY], i_qp );//反量化
                        if( b_decimate && i_decimate_8x8 < 6 )
                            i_decimate_8x8 += h->quantf.decimate_score16( h->dct.luma4x4[idx] );//有一个评分？
                        cbp = 1;
                    }
                }
            ...
            if( b_decimate )
            {
                if( i_decimate_mb < 6 )//是不是低于一定数量的非0时，就当作全部为0了，这样可以提高编码效率，对质量影响不大？
                {
                    h->mb.i_cbp_luma = 0;
                    *(uint32_t*)&h->mb.cache.non_zero_count[x264_scan8[ 0]] = 0;
                    *(uint32_t*)&h->mb.cache.non_zero_count[x264_scan8[ 2]] = 0;
                ...
                else
                {
                    for( i8x8 = 0; i8x8 < 4; i8x8++ )
                        if( h->mb.i_cbp_luma&(1<<i8x8) )
                            h->dctf.add8x8_idct( &h->mb.pic.p_fdec[0][(i8x8&1)*8 + (i8x8>>1)*8*FDEC_STRIDE], &dct4x4[i8x8*4] );	//残差进行DCT反变换之后，叠加到预测数据上
                }
            }
        }
    }
    ...
    if( IS_INTRA( h->mb.i_type ) )
    {
        ...
        {
            h->predict_8x8c[i_mode]( h->mb.pic.p_fdec[1] );//重构
            h->predict_8x8c[i_mode]( h->mb.pic.p_fdec[2] );
        }
    }
