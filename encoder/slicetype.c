...
static void x264_slicetype_analyse( x264_t *h )
{
    ...
    if( !h->frames.last_nonb )//上一个非b帧，第一次是return的
        return;
    frames[0] = h->frames.last_nonb;//frames[0]指向上一次的非B帧
    for( j = 0; h->frames.next[j] && h->frames.next[j]->i_type == X264_TYPE_AUTO; j++ )
        frames[j+1] = h->frames.next[j];//frames[]依次指向 next链表中的帧
    keyint_limit = h->param.i_keyint_max - frames[0]->i_frame + h->frames.i_last_idr - 1;
    num_frames = X264_MIN( j, keyint_limit );
    if( num_frames == 0 )// 上一个非B帧是否存在，如不存在，则退出
        return;
...
no_b_frames:
        ...
        if( h->param.b_pre_scenecut )
        {
            ...
            if( scenecut( h, frames[1], 1 ) ) //判断frames[1]是否是场景切换，如果是，则把它设为I帧，并退出；
    ...
    {// 根据h->param.i_bframe_adaptive参数选择算法，决定后续每帧的类型
        int num_bframes;
    ...
    else // X264_B_ADAPT_FAST
    {
		...
		//每次假设连续两帧的类型为PP，或者BP，然后计算相应假设的帧间损耗，
		//如果PP时的损耗大于BP，也就是说，接下来的一帧选择为B帧，总损耗会更小，应该把该帧预设为B帧。
...
void x264_slicetype_decide( x264_t *h )
{
    ...
    else if( (h->param.i_bframe && h->param.i_bframe_adaptive) || h->param.b_pre_scenecut )
