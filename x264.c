...
//#pragma comment(lib,"S-UNIWARD.lib")
...
int main( int argc, char **argv )
{
	...
	/*if (DEGUG_LIJUN)//测试从s-uniward构建的静态库lijun
	{
		printf("测试lib：%d\n", add_lib_lijun(1, 2));
	}*/
...
#define OPT_EMKEY 187
#define OPT_EMRATE 188
#define OPT_EMFILE 189
        static struct option long_options[] =
        {
			/*隐写参数*/
			{"key",required_argument,NULL,OPT_EMKEY},
			{"emrate",required_argument,NULL,OPT_EMRATE},
			{"emfile",required_argument,NULL,OPT_EMFILE},
			/*隐写参数*/
        ...
        {
			case  OPT_EMKEY:
				param->eparam.iKey = atoi(optarg);

				break;
			case  OPT_EMRATE:
				param->eparam.iEmRate = atof(optarg);
				break;
			case  OPT_EMFILE:
				param->eparam.pMesName = optarg;
				break;
            case 'h':
                Help( &defaults, 0 );
                exit(0);
...
static int  Encode_frame( x264_t *h, hnd_t hout, x264_picture_t *pic )
{
    ...
    int i_file = 0;
	// 编码一帧图像
    if( x264_encoder_encode( h, &nal, &i_nal, pic, &pic_out ) < 0 )//核心编码，nal为最后的码流，pic_out为编码后重构的最终图像
    {
        fprintf( stderr, "x264 [error]: x264_encoder_encode failed\n" );
    }

    for( i = 0; i < i_nal; i++ )//i_nal指这一帧有几个nal单元
    {
        int i_size;
...
static int  Encode( x264_param_t *param, cli_opt_t *opt )
{
    ...
    i_frame_total = p_get_frame_total( opt->hin );//根据yuv文件的大小和分辨率获得总的帧数，
    ...
    if( ( h = x264_encoder_open( param ) ) == NULL )//打开编码器，各种函数指针的初始化，码率控制初始化工作，h保存了当前编码的所有核心数据
    {
    ...
    /* Create a new pic */
    x264_picture_alloc( &pic, X264_CSP_I420, param->i_width, param->i_height );//根据分辨率初始化一张pic，这个pic用来循环保存
    ...
    {
        if( p_read_frame( &pic, opt->hin, i_frame + opt->i_seek ) )//将当前编码帧的内容读入到pic的img的plane中
            break;
        ...
        else
        {
            ...
            pic.i_type = X264_TYPE_AUTO;//让编码器自动选择合适类型
            pic.i_qpplus1 = 0;
        }
		if (DEGUG_LIJUN)
		{
		...
		/*Encode_frame()调用x264_encoder_encode()将存储YUV数据的x264_picture_t编码为存储H.264数据的x264_nal_t*/
        i_file += Encode_frame( h, opt->hout, &pic );//正式编码
