...
typedef struct x264_param_t
{
    ...
    struct
    {
        ...
        int          b_transform_8x8; // 是否使用8x8DCT
        ...
        int          i_noise_reduction; /* adaptive pseudo-deadzone 自适应位盲区，减少噪声，0意味着关闭，对于噪声很大的内容你需要打开*/
        float        f_psy_rd; /* Psy RD strength Psy优化开关，可能会增强细节*/
        float        f_psy_trellis; /* Psy trellis strength */

        /* the deadzone size that will be used in luma quantization */
        int          i_luma_deadzone[2]; /* {inter, intra} */

        int          b_psnr;    /* compute and print PSNR stats 计算和打印PSNR信息*/
    ...
    struct
    {
        ...
        int         i_vbv_buffer_size;// 置VBV可用的最大缓冲区，单位是kbits,码率控制缓冲区的大小，单位kbit，默认0
        ...
        char        *psz_stat_in; // 输入文件,文件存有第一次编码的统计数据
	...
	struct {
		...
		uint8_t * InfDump;
		uint32_t iMesLen;//秘密信息文件字节数
		uint32_t iSumOffset;//所有需要嵌入的信息中，当前bit偏移
	}eparam;
...
typedef struct
{
    ...
    int     i_plane;
    int     i_stride[4];
    uint8_t *plane[4];//存储原始的数据，[0]是Y分量 ，按照一个大矩阵存储lijun，[1][2]是UV分量
} x264_image_t;
